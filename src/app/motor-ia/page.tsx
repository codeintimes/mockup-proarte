...
export default function MotorIA() {
  return (
    <div className="p-4 sm:p-6 max-w-[1400px] fade-in">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-1">
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-lg sm:text-xl font-bold text-slate-900">Motor IA</h1>
      ...
      {/* Comm Stats Row — 2 cols on mobile, 4 on md+ */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-5">
        {[
          ...
          <div key={label} className="bg-white border border-slate-200 rounded-xl shadow-sm p-3 sm:p-4">
            <div className="flex items-start justify-between mb-2">
              <span className="text-xs text-slate-500 leading-snug pr-1">{label}</span>
              <div className={clsx(
      ...
      {/* Main content: stacked on mobile, 3-col on lg */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        {/* Left: Blockers + Friction + Recommendations */}
        <div className="lg:col-span-2 space-y-5">
                ...
                <div key={b.id} className="px-4 py-3 hover:bg-slate-50 transition-colors">
                  <div className="flex flex-wrap items-center gap-2 mb-1.5">
                    <div className={clsx(
                      ...
                      b.severity === "high" ? "bg-amber-50 text-amber-700" : "bg-slate-100 text-slate-600"
                    )}>
                      {b.from}
                    </div>
                    <ArrowRight size={14} className="text-slate-300 shrink-0" />
                    <div className="text-xs text-slate-500">
                      esperando a <span className="font-semibold text-slate-700">{b.to}</span>
                    </div>
          ...
          {/* Friction — table on md+, cards on mobile */}
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm">
              ...
              {frictionAlerts.map((f) => (
                <div key={f.id} className="px-4 py-3 space-y-1">
                  <div className="flex items-start gap-2">
                    ...
                    <span className="text-xs font-medium text-slate-700">{f.problem}</span>
                  </div>
                  <div className="flex flex-wrap gap-2 pl-5 text-xs text-slate-500">
                    <span>{f.team}</span>
                    <span>·</span>
              ...
              {aiRecommendations.map((rec) => (
                <div key={rec.id} className="border border-slate-200 rounded-xl p-3 hover:border-indigo-200 hover:bg-indigo-50/20 transition-colors group">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button className="text-xs text-indigo-600 hover:text-indigo-700 flex items-center gap-0.5 font-medium">
                  ...
                  {lastMeeting.generatedTasks.map((t, i) => (
                    <div key={i} className={clsx(
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-slate-400 mt-0.5">{t.assignee} · vence {t.due}</div>
                      </div>
                    </div>
                  ))}
                </div>
