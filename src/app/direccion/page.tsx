  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-3 sm:p-4">
      <div className="flex items-start justify-between mb-2">
        ...
        <span className={clsx("flex items-center gap-0.5 text-xs font-medium px-1.5 py-0.5 rounded shrink-0", trendBg, trendColor)}>
          <TrendIcon value={trend} />
          {trend > 0 ? "+" : ""}{trend}{unit === "/5" ? "" : "%"}
        </span>
      </div>
      <div className={clsx("text-xl sm:text-2xl font-bold mb-1", color)}>
        {value}{unit}
      </div>
      <div className="h-8 sm:h-10">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={sparkPoints}>
            <Line type="monotone" dataKey="v" stroke={trend >= 0 ? "#10b981" : "#ef4444"} strokeWidth={1.5} dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>
  ...
  return (
    <div className="p-4 sm:p-6 max-w-[1400px] fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-5">
        <KpiCard label={kpis.teamHealth.label} value={kpis.teamHealth.value} unit={kpis.teamHealth.unit} trend={kpis.teamHealth.trend} sparkData={sparklines.teamHealth} color="text-amber-600" />
        <KpiCard label={kpis.avgLoad.label} value={kpis.avgLoad.value} unit={kpis.avgLoad.unit} trend={kpis.avgLoad.trend} sparkData={sparklines.avgLoad} color="text-amber-500" />
        <KpiCard label={kpis.taskCompletion.label} value={kpis.taskCompletion.value} unit={kpis.taskCompletion.unit} trend={kpis.taskCompletion.trend} sparkData={sparklines.taskCompletion} color="text-emerald-600" />
        <KpiCard label={kpis.emotionalIndex.label} value={kpis.emotionalIndex.value} unit={kpis.emotionalIndex.unit} trend={kpis.emotionalIndex.trend} sparkData={sparklines.emotionalIndex} color="text-indigo-600" />
      </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">% de carga · línea roja = umbral crítico (85%)</p>
              </div>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={chartData} layout="vertical" margin={{ left: 10, right: 30, top: 0, bottom: 0 }}>
                ...
                <YAxis type="category" dataKey="name" tick={{ fontSize: 11, fill: "#475569" }} width={75} />
                <Tooltip
                  formatter={(v) => [`${v}%`, "Carga"]}
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-3">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">Datos anónimos y agregados</p>
              </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs min-w-[400px]">
                <thead>
                  <tr>
                    <th className="text-left text-slate-400 font-medium pb-2 pr-3 w-24">Equipo</th>
